module.exports = function(app) {
    var bookingFlightController = function ($state,$stateParams,dataFactory,$uibModal,carUtilities, flightUtilities) {
        ...
        vm.flightUtilities = flightUtilities;
        ...
        function init() {
            ...
            var successFunc = function (response) {
                ...
                //good!
                vm.details = response.data.data.results.results;
            ...
            dataFactory.getFlightBooking(params).then(successFunc,errFunc);
        ...
        //vm.openTaxesAndFeesModal = function() {
            //flightUtilities.setupModal(vm.details.popup_data.taxes_and_fees);
        ...
        function setSubstitutes(p) {
            p = p.replace('#flight_schedule_START#Flight Schedule#flight_schedule_END#','<a href="https://secure.rezserver.com/flights/help/review/?refid=7760" target="_blank">Flight Schedule</a>');
            return p;
            //return carUtilities.setSubstitutes(p,'booking');
        }

        vm.getPopupTag = function(text) {
            var indexOfSecond = text.indexOf('#',1);
            var f =  text.substring(1,indexOfSecond);
            var v = f.replace('_START','');
            return v;
        };

        vm.getPopupTitle = function(text) {
            var idx2 = text.indexOf('#',1);
            var idx3 = text.indexOf('#',idx2+1);
            return text.substring(idx2+1,idx3);
    ...
    bookingFlightController.$inject = ['$state','$stateParams','dataFactory','$uibModal','carUtilities', 'flightUtilities'];
    app.controller('bookingFlightController', bookingFlightController);
};
